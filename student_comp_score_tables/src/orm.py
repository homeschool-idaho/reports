...
IOWA_SCORE_DATA_COL_NAMES =  (
   'score_id', 'reading', 'vocabulary', 'spelling', 'capitalization',
   'punctuation', 'language_or_written_exp', 'conventions_of_writ', 'ela_total', 'word_analysis', 'listening',
   'ext_ela_total', 'mathematics', 'computation', 'math_Total',
   'core_comp_ela', 'core_comp_ext_ela', 'core_comp_ela_nocomp', 'core_comp_ext_ela_nocomp', 'social_studies',
   'science', 'cplte_comp_ela', 'cplte_comp_ext_ela', 'cplte_comp_ela_nocomp', 'cplte_comp_ext_ela_nocomp',
   'reading_words', 'reading_comprehension', 'reading_total', 'language_total', 'not_used_1', 'not_used_2')
NUM_IOWA_SCORE_DATA_COLS = len(IOWA_SCORE_DATA_COL_NAMES)
class iowa_score_data:
   def __init__(self, row):
      self.score_id          = float(row[ 0])
      self.reading           = float(row[ 1])
      self.vocabulary        = float(row[ 2])
      self.spelling          = float(row[ 3])
      self.capitalization    = float(row[ 4])
      self.punctuation       = float(row[ 5])
      self.language_or_written_exp = float(row[ 6])
      self.conventions_of_writ     = float(row[ 7])
      self.ela_total         = float(row[ 8])
      self.word_analysis     = float(row[ 9])
      self.listening         = float(row[10])
      self.ext_ela_total     = float(row[11])
      self.mathematics       = float(row[12])
      self.computation       = float(row[13])
      self.math_Total        = float(row[14]) #Math Total
      self.core_comp_ela     = float(row[15]) #Composite Total
      self.core_comp_ext_ela = float(row[16])
      self.core_comp_ela_nocomp     = float(row[17])
      self.core_comp_ext_ela_nocomp = float(row[18])
      self.social_studies    = float(row[19])
      self.science           = float(row[20])
      self.cplte_comp_ela    = float(row[21])
      self.cplte_comp_ext_ela    = float(row[22])
      self.cplte_comp_ela_nocomp = float(row[23])
      self.cplte_comp_ext_ela_nocomp = float(row[24])
      self.reading_words     = float(row[25])
      self.reading_comprehension = float(row[26])
      self.reading_total     = float(row[27])
      self.language_total    = float(row[28])
      self.not_used_1        = float(row[29])
      self.not_used_2        = float(row[30])
   #
#
