...
def GetDbScores(score_type, test_year):
	...
	return [compositeAvg, vocabularyAvg, readingAvg, languageAvg, mathematicsAvg, sourcesAvg], numTotalStudents, numSummedStudents
...
def MakePdfChart(datafile, outfile):
	...
	if bFlatChart:
		...
		c.rect( ((width-rwidth)/2) + barWidth/2 , y, rwidth, rheight, stroke=1, fill=0)   # x, y, width, height, stroke, fill
	#
		...
		for barNum in range( len(chart_data) ):
			...
			if barNum < 3:
				...
				if bFlatChart:
					...
					#   Right side
					p = c.beginPath()
					p.moveTo(barX + barWidth - barWidth/3, y - barWidth/3)   # + barWidth/3 to override colored line immediately above
					p.lineTo(barX + barWidth - barWidth/3, y + barHeight - barWidth/3)
					p.lineTo(barX + barWidth,              y + barHeight)
